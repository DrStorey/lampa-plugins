/**
 * Anime Catalog for Lampa v4.0.0
 *
 ...
 *
 * Возможности:
 * - Каталог Shikimori API с пагинацией, фильтрами и сортировкой а так же темами
 ...
 *
 * @author kartmansms
 * @Данная версия изменена и переписана автором TheStorey
 * @license MIT
 */
...
(function () {
    ...
    if (window.plugin_anime_catalog_ready) return;
    window.plugin_anime_catalog_ready = true;

    var SETTINGS_KEY = 'lampa_anime_settings_v1';
    ...
    /**
     * Подготовить список жанров для панели фильтров.
     * Исходный плагин вызывал эту функцию, но не содержал её реализации.
     */
    function filterGenres(genres) {
        ...
        for (var i = 0; i < genres.length; i++) {
            var genre = genres[i];
            if (!genre || !genre.id || seen[String(genre.id)]) continue;
            if (String(genre.kind || '').toLowerCase() !== 'genre') continue;

            var name = String(genre.name || '').toLowerCase().trim();
            var russian = String(genre.russian || '').toLowerCase().trim();
            var isAdult = !!(adultGenres[name] || adultNames[name] || adultNames[russian]);

            if (settings.hide_adult && isAdult) continue;

            seen[String(genre.id)] = true;
            result.push(genre);
        }

        result.sort(function (a, b) {
            var left = String(a.russian || a.name || '').toLowerCase();
            var right = String(b.russian || b.name || '').toLowerCase();
    ...
    /** Отделить темы Shikimori от жанров и демографии. */
    function filterThemes(genres) {
        var result = [];
        var seen = {};
        ...
        for (var i = 0; i < genres.length; i++) {
            var theme = genres[i];
            if (!theme || !theme.id || seen[String(theme.id)]) continue;
            if (String(theme.kind || '').toLowerCase() !== 'theme') continue;

            seen[String(theme.id)] = true;
            result.push(theme);
        }
    ...
    function loadGenres(callback) {
        ...
        gqlQuery('{ genres(entryType: Anime) { id name russian kind } }', function (data) {
    ...
    /** Загрузить только темы Shikimori, используя общий кеш классификаторов. */
    function loadThemes(callback) {
        var cache = storageGet(GENRES_CACHE_KEY, null);
        var DAY_MS = 86400000;

        if (cache && cache.genres && cache.genres.length && cache.ts && (Date.now() - cache.ts) < DAY_MS) {
            callback(filterThemes(cache.genres));
            return;
        }
        ...
        gqlQuery('{ genres(entryType: Anime) { id name russian kind } }', function (data) {
            ...
            storageSet(GENRES_CACHE_KEY, { genres: data.genres, ts: Date.now() });
            callback(filterThemes(data.genres));
        }, function () {
            callback([]);
        });
    }
    ...
    function requestAnime(params, oncomplete, onerror) {
        ...
        var genreAndTheme = [];
        if (params.genre) genreAndTheme.push(params.genre);
        if (params.theme) genreAndTheme.push(params.theme);
        if (genreAndTheme.length) args.push('genre: "' + genreAndTheme.join(',') + '"');
            ...
            var element = $(
                '<div class="card Shikimori selector' + compact + '" data-id="' + esc(data.id) + '">' +
                    '<div class="card__view">' +
                        ...
                        '<div class="Shikimori-card__status Shikimori-card__status--' + esc(data.status || '') + '">' + esc(statusName(data.status)) + '</div>' +
    ...
    function Catalog(object) {
        ...
        this.render = function () {
            if (!rendered) {
                ...
                html.append(quick).append(head).append(active).append(scroll.render());
                scroll.append(body);
                scroll.minus();
        ...
        this.start = function () {
            ...
            setTimeout(function () {
                if (catalogActive) restoreCatalogNavigation();
            }, 80);
        ...
        this.stop = function () {
            if (catalogActive && scroll.position) savedScrollPosition = scroll.position();
            catalogActive = false;
        };
        ...
        function restoreCatalogNavigation() {
            if (!catalogActive || !html.length) return;

            var focusTarget = null;
            var visibleSelectors = html.find('.selector:visible');

            if (!visibleSelectors.length) return;

            if (lastCardId) focusTarget = html.find('.selector[data-id="' + lastCardId + '"]')[0];
            if (!focusTarget) focusTarget = last;
            if (!focusTarget || focusTarget.offsetParent === null) focusTarget = visibleSelectors.first()[0];

            Lampa.Controller.collectionSet(html, false, true);
            if (focusTarget) Lampa.Controller.collectionFocus(focusTarget, html, true);

            if (scroll.position && scroll.shift) {
                scroll.shift(scroll.position() - savedScrollPosition);
            } else if (scroll.restorePosition) {
        ...
        function buildAnimeHeader() {
            quick.append('<div class="Shikimori-section-label">Жанры</div>');
            addGenreQuick('Все', '', '');
            renderGenreBar();

            addHeadButton('Поиск', iconSearch(), openSearch, !!params.search);
            addHeadButton('Сортировка: ' + sortName(params.sort || readSettings().default_sort), iconSort(), function () {
                openFilterSortMenu();
            }, params.sort && params.sort !== readSettings().default_sort);
            addHeadButton('Год: ' + (params.season ? seasonName(params.season) : 'все'), iconCalendar(), openYears, !!params.season);
            addHeadButton('Статус: ' + (params.status ? statusName(params.status) : 'все'), iconStatus(), function () {
                openFilterStatusMenu();
            }, !!params.status);
            addHeadButton('Темы: ' + (selectedThemeIds().length || 'все'), iconThemes(), function () {
                loadThemes(openFilterThemeMenu);
            }, !!params.theme);
            addHeadButton('Тип: ' + (params.kind ? kindName(params.kind) : 'все'), iconType(), function () {
                openFilterKindMenu();
            }, !!params.kind);

            if (isAuthorized()) addHeadButton('Мой список', iconUser(), openProfile, !!params.mylist);
            addHeadButton('Настройки', iconSettings(), openSettings, false, true);

            if (hasCatalogSelection()) {
                addHeadButton('Сбросить', iconReset(), function () {
                    openWith({
                        page: 1,
                        sort: readSettings().default_sort,
                        search: '',
        ...
        function hasCatalogSelection() {
            return params.search || params.kind || params.status || params.season || params.genre || params.theme || params.mylist ||
                (params.sort && params.sort !== readSettings().default_sort);
        }

        function selectedGenreIds() {
            var raw = String(params.genre || '').split(',');
            var result = [];
        ...
        function selectedGenreTitles() {
            if (!params.genre_title) return [];
            return String(params.genre_title).split(' • ');
        }

        function selectedThemeIds() {
            var raw = String(params.theme || '').split(',');
            var result = [];
        ...
        function hasGenre(id) {
            return selectedGenreIds().indexOf(String(id)) !== -1;
        }

        function toggleGenre(id, title) {
            id = String(id || '');

            if (!id) {
                openWith({ genre: '', genre_title: '', page: 1 });
                return;
            }

            var ids = selectedGenreIds();
            var titles = selectedGenreTitles();
            ...
            openWith({
                genre: ids.join(','),
                genre_title: titles.join(' • '),
                page: 1
            });
        }

        function addGenreQuick(title, id, genreTitle) {
            var selected = id ? hasGenre(id) : !selectedGenreIds().length;
            ...
            bindPress(btn, function () { toggleGenre(id, genreTitle || title); });
            quick.append(btn);
        }

        function renderGenreBar() {
            loadGenres(function (genres) {
                var preferred = ['action', 'adventure', 'comedy', 'romance', 'fantasy', 'science fiction', 'drama', 'thriller', 'detective'];
                var added = {};
                var addedCount = 0;

                function appendGenre(genre) {
                    if (!genre || !genre.id || added[genre.id]) return;
                    added[genre.id] = true;
                    addedCount += 1;
                    addGenreQuick(genre.russian || genre.name, String(genre.id), genre.russian || genre.name);
                }

                for (var p = 0; p < preferred.length; p++) {
                    for (var i = 0; i < genres.length; i++) {
                        if (String(genres[i].name || '').toLowerCase() === preferred[p]) appendGenre(genres[i]);
                    }
                }

                for (var j = 0; j < genres.length && addedCount < 9; j++) appendGenre(genres[j]);

                var count = selectedGenreIds().length;
                var allTitle = count ? 'Все жанры (' + count + ')' : 'Все жанры';
                var all = $('<div class="simple-button selector Shikimori-chip Shikimori-chip--more">' + esc(allTitle) + ' <span>›</span></div>');
                all.on('hover:focus nav_focus', function () { last = all[0]; });
                bindPress(all, function () { openFilterGenreMenu(genres); });
                quick.append(all);

                if (catalogActive && window.Lampa && Lampa.Controller) {
                    Lampa.Controller.collectionSet(html, false, true);
                }
            });
        }

        function iconSearch() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg>'; }
        function iconSort() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7h11M4 12h8M4 17h5"/><path d="m17 14 3 3 3-3M20 6v11"/></svg>'; }
        function iconCalendar() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M16 3v4M8 3v4M3 10h18"/></svg>'; }
        function iconStatus() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="m8 12 3 3 5-6"/></svg>'; }
        function iconThemes() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3a4 4 0 0 0-4 4v1H7a4 4 0 0 0 0 8h1v1a4 4 0 0 0 8 0v-1h1a4 4 0 0 0 0-8h-1V7a4 4 0 0 0-4-4Z"/><path d="M9 12h6"/></svg>'; }
        function iconType() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m10 9 5 3-5 3z"/></svg>'; }
        function iconUser() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M5 21a7 7 0 0 1 14 0"/></svg>'; }
        function iconSettings() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M4.9 4.9 7 7M17 17l2.1 2.1M2 12h3M19 12h3M4.9 19.1 7 17M17 7l2.1-2.1"/></svg>'; }
        function iconReset() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12a9 9 0 1 0 3-6.7L3 8"/><path d="M3 3v5h5"/></svg>'; }
        ...
        function addHeadButton(title, iconSvg, action, selected, muted) {
            var stateClass = selected ? ' Shikimori-head__button--active' : '';
            if (muted) stateClass += ' Shikimori-head__button--muted';
            var btn = $('<div class="simple-button selector Shikimori-head__button' + stateClass + '">' + iconSvg + '<span>' + esc(title) + '</span></div>');
        ...
        function renderActive() {
            ...
            if (params.genre) parts.push('жанры: ' + (params.genre_title || params.genre));
            if (params.theme) parts.push('темы: ' + (params.theme_title || params.theme));
            if (params.sort && params.sort !== readSettings().default_sort) parts.push('сортировка: ' + sortName(params.sort));

            active.html(parts.length ? '<span>Выбрано:</span> ' + esc(parts.join(' / ')) : '<span>Аниме</span> — популярное сейчас');
        }
        ...
        function openSearch(btnElement) {
            ...
            if (window.Lampa && Lampa.Input && Lampa.Input.edit) {
                ...
                value = window.prompt('Поиск аниме', value);
        ...
        function openFilterKindMenu(genres) {
            ...
            var items = [
                ...
                { title: selectedTitle(current === 'tv', 'Сериал'), value: 'tv' },
                { title: selectedTitle(current === 'movie', 'Фильм'), value: 'movie' },
                { title: selectedTitle(current === 'ova', 'OVA'), value: 'ova' },
                { title: selectedTitle(current === 'ona', 'ONA'), value: 'ona' },
                { title: selectedTitle(current === 'special', 'Спешл'), value: 'special' }
            ];
        ...
        function openFilterStatusMenu(genres) {
            ...
            var items = [
                { title: selectedTitle(!current, 'Любой'), value: '' },
                { title: selectedTitle(current === 'ongoing', 'Выходит сейчас'), value: 'ongoing' },
                { title: selectedTitle(current === 'anons', 'Анонс'), value: 'anons' },
                { title: selectedTitle(current === 'released', 'Завершён'), value: 'released' }
            ];
        ...
        function openFilterGenreMenu(genres) {
            var items = [];
            var draftIds = selectedGenreIds();
            var draftTitles = [];
            var applied = false;

            function titleById(id) {
                for (var g = 0; g < genres.length; g++) {
                    if (String(genres[g].id) === String(id)) return genres[g].russian || genres[g].name || String(id);
                }
                return String(id);
            }

            for (var d = 0; d < draftIds.length; d++) draftTitles.push(titleById(draftIds[d]));

            function changeDraft(id, title, checked) {
                id = String(id);
                var index = draftIds.indexOf(id);

                if (checked && index === -1) {
                    draftIds.push(id);
                    draftTitles.push(title || id);
                } else if (!checked && index !== -1) {
                    draftIds.splice(index, 1);
                    draftTitles.splice(index, 1);
                }
            }
            ...
            function applyDraft() {
                ...
                var nextIds = draftIds.join(',');
                var nextTitles = draftTitles.join(' • ');

                if (nextIds === selectedGenreIds().join(',')) {
                    Lampa.Controller.toggle('content');
                    return;
                }

                openWith({ genre: nextIds, genre_title: nextTitles, page: 1 });
            }

            items.push({
                title: 'Сбросить все жанры',
                value: 'clear_genres'
            });
            ...
            items.push({ title: 'Применить выбранные жанры', value: 'apply' });

            Lampa.Select.show({
                title: 'Жанры — отметьте нужные',
                items: items,
                onCheck: function (item) {
                    changeDraft(item.value, item.genre_title, !!item.checked);
                },
                onSelect: function (item) {
                    if (item.value === 'noop') return;

                    if (item.value === 'clear_genres') {
                        applied = true;
                        openWith({ genre: '', genre_title: '', page: 1 });
        ...
        function openFilterThemeMenu(themes) {
            var items = [];
            var draftIds = selectedThemeIds();
            var draftTitles = [];
            var applied = false;

            function titleById(id) {
                for (var t = 0; t < themes.length; t++) {
                    if (String(themes[t].id) === String(id)) return themes[t].russian || themes[t].name || String(id);
                }
                return String(id);
            }
            ...
            function applyDraft() {
                ...
                if (nextIds === selectedThemeIds().join(',')) {
                    Lampa.Controller.toggle('content');
                    return;
                }

                openWith({ theme: nextIds, theme_title: nextTitles, page: 1 });
            }

            items.push({ title: 'Сбросить все темы', value: 'clear_themes' });

            for (var i = 0; i < themes.length; i++) {
                if (!themes[i] || !themes[i].id) continue;

                var themeTitle = themes[i].russian || themes[i].name || themes[i].id;
                items.push({
                    title: themeTitle,
                    value: String(themes[i].id),
                    theme_title: themeTitle,
                    checkbox: true,
                    checked: draftIds.indexOf(String(themes[i].id)) !== -1
                });
            }

            if (items.length === 1) items.push({ title: 'Темы недоступны', value: 'noop' });

            items.push({ title: '', separator: true });
            items.push({ title: 'Применить выбранные темы', value: 'apply' });

            Lampa.Select.show({
                title: 'Темы — отметьте нужные',
                items: items,
                onCheck: function (item) {
                    changeDraft(item.value, item.theme_title, !!item.checked);
                },
                onSelect: function (item) {
                    if (item.value === 'noop') return;

                    if (item.value === 'clear_themes') {
                        applied = true;
                        openWith({ theme: '', theme_title: '', page: 1 });
                    } else if (item.value === 'apply') {
                        applyDraft();
                    }
                },
        ...
        function openYears() {
            var current = params.season || '';
            var year = new Date().getFullYear();
            var items = [{ title: selectedTitle(!current, 'Любой год'), value: '' }];

            for (var i = 0; i < 12; i++) {
                var value = String(year - i);
                items.push({ title: selectedTitle(current === value, value), value: value });
            }

            var ranges = [
                { title: '2010–2014', value: '2010_2014' },
                { title: '2000–2009', value: '2000_2009' },
                { title: '1990–1999', value: '1990_1999' },
                { title: 'До 1990', value: '1917_1989' }
            ];

            for (var r = 0; r < ranges.length; r++) {
                ranges[r].title = selectedTitle(current === ranges[r].value, ranges[r].title);
                items.push(ranges[r]);
            }
            ...
            Lampa.Select.show({
                ...
                items: items,
                onSelect: function (item) {
                    openWith({ season: item.value, page: 1 });
                },
                onBack: function () {
                    Lampa.Controller.toggle('content');
                }
        ...
        function load(append) {
            ...
            requestAnime(params, function (data) {
                ...
                function renderCards() {
                    ...
                    if (catalogActive && window.Lampa && Lampa.Controller) {
                        Lampa.Controller.collectionSet(html, false, true);
                        Lampa.Controller.collectionFocus(last || body.find('.selector:visible').first(), html, true);
                    }
                }
    ...
    function fullPage() {
        var page = $('.full-start-new:visible, .full-start:visible, .full:visible').last();
        return page && page.length ? page : $();
    }

    function isFullActivity(activity) {
        activity = normalizeActivity(activity || {});
        return !!(activity && activity.component === 'full');
    }
    ...
    function scheduleAppendFull(activity) {
        ...
        delays.forEach(function (delay) {
            setTimeout(function () {
                if (!isFullActivity(getActiveActivity())) return;
                ...
                if (!apiCalled) {
                    ...
                    resolveShikiAnimeForFull(activity || getActiveActivity(), function (anime) {
                        if (anime && anime.id && isFullActivity(getActiveActivity()) && fullPage().length) {
    ...
    function extendFull() {
        ...
        Lampa.Listener.follow('full', function (event) {
            ...
            fullPollId = setInterval(function () {
                if (!isFullActivity(getActiveActivity())) {
        ...
        Lampa.Listener.follow('activity', function (event) {
            ...
            if (!isFullActivity(activity)) activity = getActiveActivity();

            if (isFullActivity(activity)) scheduleAppendFull(activity);
            else if (fullPollId) { clearInterval(fullPollId); fullPollId = null; }
        });
    }
    ...
    function appendFull(activity, anime) {
        ...
        if (!anime || !anime.id || !page.length || !isFullActivity(getActiveActivity())) return;
        ...
        if (!page.find('.shikimori-full-list-button').length) {
            ...
            setTimeout(function () {
                try {
                    if (window.Lampa && Lampa.Controller && isFullActivity(getActiveActivity()) && page.is(':visible')) {
    ...
    function addStyles() {
        ...
        $('body').append(
            '<style id="shikimori-style">' +
                ...
                /* Anime catalog v4: compact native-looking filter rail */
                '.Shikimori-module{padding:.45em 1.1em 2em;background:rgba(17,19,21,.2)}' +
                '.Shikimori-quick{display:flex;align-items:center;flex-wrap:wrap;gap:.38em;margin:0 0 .55em;padding:.15em 0 .65em;border-bottom:1px solid rgba(255,255,255,.08)}' +
                '.Shikimori-section-label{font-size:1.05em;font-weight:700;margin-right:.35em;color:#fff;letter-spacing:.01em}' +
                '.Shikimori-head{display:flex;align-items:center;flex-wrap:wrap;gap:.42em;margin:0 0 .35em}' +
                '.Shikimori-head__button,.Shikimori-chip{margin:0!important;border:1px solid rgba(255,255,255,.10)!important;background:rgba(255,255,255,.065)!important;color:rgba(255,255,255,.86)!important}' +
                '.Shikimori-head__button{padding:.58em .82em!important;border-radius:.42em!important;font-size:.88em!important}' +
                '.Shikimori-head__button--active{background:rgba(232,75,99,.2)!important;border-color:rgba(232,75,99,.5)!important;color:#ff9aaa!important}' +
                '.Shikimori-head__button--muted{opacity:.72}' +
                '.Shikimori-chip{padding:.42em .82em!important;border-radius:1.2em!important;font-size:.84em!important}' +
                '.Shikimori-chip--more span{font-size:1.35em;line-height:.5;margin-left:.25em}' +
                '.Shikimori-head__button.focus,.Shikimori-chip.focus,.Shikimori-more.focus{background:#fff!important;color:#17191c!important;border-color:#fff!important;transform:scale(1.035);box-shadow:0 .28em 1em rgba(0,0,0,.38)!important}' +
                '.Shikimori-chip--active:not(.focus){background:#e84b63!important;border-color:#e84b63!important;color:#fff!important}' +
                '.Shikimori-active{font-size:.84em;margin:.1em 0 .55em;color:rgba(255,255,255,.48)}' +
                '.Shikimori-active span{color:#ff7187}' +
                '.Shikimori-body{padding:.55em 0 1em;margin:0 -.48em}' +
                '.Shikimori.card{flex:0 0 16.666%;max-width:16.666%;padding:0 .48em;margin:0 0 1.15em}' +
                '.Shikimori.card.Shikimori--compact{flex:0 0 12.5%;max-width:12.5%}' +
                '.Shikimori.card .card__view{border-radius:.52em;background:#202226;padding-bottom:145%;transition:transform .16s ease,box-shadow .16s ease}' +
                '.Shikimori.card.focus .card__view{transform:translateY(-.15em);box-shadow:0 0 0 .2em #fff,0 .55em 1.5em rgba(0,0,0,.52)}' +
                '.Shikimori-card__badge{left:.42em;right:auto;top:.42em;background:#ff495c;color:#fff;font-weight:700}' +
                '.Shikimori-card__status{position:absolute;left:.42em;top:2.15em;padding:.28em .48em;border-radius:.25em;background:#f5a900;color:#17120a;font-size:.72em;line-height:1;font-weight:700;text-transform:capitalize}' +
                '.Shikimori-card__status--ongoing{background:#22b9e6;color:#07151b}' +
                '.Shikimori-card__status--anons{background:#a98aff;color:#100b1e}' +
                '.Shikimori-card__status:empty{display:none}' +
                '.Shikimori-card__rating{left:auto;right:.45em;top:auto;bottom:.45em;background:rgba(17,18,20,.82);font-weight:800;color:#fff}' +
                '.Shikimori-card__user-rate{top:auto;bottom:.45em;left:.45em}' +
                '.Shikimori.card .card__title{font-size:.98em;line-height:1.2;max-height:2.45em;margin-top:.48em;font-weight:500}' +
                '.Shikimori-card__meta{font-size:.78em;height:1.4em;margin-top:.18em;color:rgba(255,255,255,.58)}' +
                '@media(max-width:1100px){.Shikimori.card{flex-basis:20%;max-width:20%}.Shikimori-head__button{font-size:.82em!important}.Shikimori-chip{font-size:.78em!important}}' +
                '@media(max-width:760px){.Shikimori-module{padding:.35em .7em 1.5em}.Shikimori.card{flex-basis:33.333%;max-width:33.333%}.Shikimori-head__button span{max-width:10em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}' +
            '</style>'
        );
    }
