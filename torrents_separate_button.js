        ...
        Lampa.Listener.follow('full', function (e) {
          if (e.type == 'complite') {
            ...
            if (e.data && e.object) {
              ...
              if (render.find('.pva-torrent-button').length) return;

              setTimeout(function () {
                var showy = render.find('.showy-ru-showy60--button').first();
                ...
                var anchor = render.find('.button--play, .view--torrent').first();

                if (anchor.length) {
                  anchor.before(btn);
                }
                else {
                  render.find('.full-start__buttons').first().append(btn);
                }
              }, 300);
            }
          }
